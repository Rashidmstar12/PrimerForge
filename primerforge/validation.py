...
class BenchmarkValidator:
    ...
    def _make_primer_pair(self, f_seq: str, r_seq: str, product_size: int = 120) -> PrimerPair:
        ...
        return PrimerPair(
            ...
            cross_dimer_dg=self.engine.calculate_heterodimer_dg(f_seq, r_seq), penalty=0.0,
        )
    ...
    def _compute_metrics(self, y_true: np.ndarray, y_scores: np.ndarray) -> Dict[str, float]:
        ...
        recall = tps[1:] / y_true_sorted.sum() if y_true_sorted.sum() > 0 else np.zeros_like(tps[1:])
    ...
    def validate_rtprimerdb(self, scorer: MLScorer) -> Dict[str, float]:
        ...
        if not os.path.exists(csv_path):
            ...
            df = pd.DataFrame({
                "forward_seq": ["ATTGGCAATGAGCGGTTCCG", "GCGCTCAGGAGGAGCAATGA", "TCCGCTGCCCTGAGGCACTC", "CACCATTGGCAATGAGCGGT"],
                "reverse_seq": ["GCGCTCAGGAGGAGCAATGA", "ATTGGCAATGAGCGGTTCCG", "GATCTTGATCTTCATTGTGCT", "CGCTCAGGAGGAGCAATGAT"],
        ...
        for _, row in df.iterrows():
            ...
            spec_data = {"f_off_targets": 0.0, "r_off_targets": 0.0, "f_var_dist": 20.0, "r_var_dist": 20.0}
    ...
    def validate_primerbank(self, scorer: MLScorer) -> Dict[str, float]:
        ...
        if not os.path.exists(csv_path):
            df = pd.DataFrame({
                "forward_seq": ["ATTGGCAATGAGCGGTTCCG", "GGAGCGAGATCCCTCCAAAT", "ACCCCACTGAAAAAAGATGA"],
                "reverse_seq": ["GCGCTCAGGAGGAGCAATGA", "GGCTGTTGTCATACTTCTCATGG", "ATCTTTTCAGTGGGGGTGAATT"],
    ...
    def validate_hard_negatives(self, scorer: MLScorer) -> float:
        ...
        failed_primers = [
            ("ATGCATGCATGCATGCATGC", "GCATGCATGCATGCATGCAT", 5.0, 1.0),  # High off-targets + SNP at 3' end
            ("AAAAAAAATTTTTTTTGGGG", "GGGGGGGGCCCCCCCCAAAA", 4.0, 1.0),  # Structural overlap + SNP
            ("GCGCGCGCGCGCGCGCGCGC", "CGCGCGCGCGCGCGCGCGCG", 3.0, 1.0)   # Self-dimers + Off-targets
        ]
    ...
    def generate_validation_report(self, scorer: MLScorer, out_dir: str = "plots") -> str:
        ...
        tpr = np.sort(tpr)
        ax.plot(fpr, tpr, color="#D32F2F", linewidth=3, label=f"RTPrimerDB (AUC={rt_metrics['roc_auc']:.3f})")
        ...
        ax.plot(x, y, "o-", color="#D32F2F", linewidth=2.0, markersize=5, label=f"RTPrimerDB (ECE={rt_metrics['ece']:.3f})")
