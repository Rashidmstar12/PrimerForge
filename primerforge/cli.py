...
def design(
    ...
    logger.info(f"Target Template: {target_sequence[:30]}..." if len(target_sequence) > 30 else f"Target Template: {target_sequence}")
    ...
    ml_scorer = MLScorer(model_path=os.path.join(model_dir, "primerforge_lightgbm.model"))
    if retrain_ultra:
        click.echo("Force retraining ultra model: Curation pipeline compiling ultra-scale ensembled databases...")
        ...
        click.echo("Force retraining hybrid model: Curation pipeline compiling real-world + synthetic databases...")
        ...
        click.echo("Force retraining: Curation pipeline compiling the 30k-pair database...")
    ...
    if tiled:
        ...
        if not selected_tiles:
            click.echo("Error: No overlapping tiled amplicons could be designed matching your specifications.")
            return

        if multiplex:
            click.echo("Running ILP multiplex optimization on tiled candidate amplicons...")
        ...
        click.echo("=" * 80)
        click.echo(f"  Reference Sequence Length: {len(target_sequence)}bp | Selected Tiles: {len(final_tiles)}")
        ...
        for r_idx, item in enumerate(final_tiles, 1):
            ...
            click.echo(f"  Forward: {pair.forward.sequence} (Tm={pair.forward.tm:.1f}°C, GC={pair.forward.gc_percent:.1f}%)")
            click.echo(f"  Reverse: {pair.reverse.sequence} (Tm={pair.reverse.tm:.1f}°C, GC={pair.reverse.gc_percent:.1f}%)")
            click.echo(f"  Product Size: {pair.product_size}bp | Cross Dimer: {pair.cross_dimer_dg:.2f} kcal/mol")
        click.echo("=" * 80 + "\n")
        return

    try:
        candidates = biophys_engine.generate_candidates(target_sequence, num_return=max(50, num_return * 5))
    except Exception as e:
        raise click.ClickException(str(e))

    if not candidates:
        click.echo("Error: No thermodynamic primer candidates found matching your specifications.")
        return
    ...
    for idx, pair in enumerate(candidates):
            ...
            if var_filter:
                for hit in hits_f:
                    ...
                    for var in var_filter.variants:
                        if hit.start <= var.pos <= hit.start + len(pair.forward.sequence):
                            if var.maf >= maf:
                                dist = (hit.start + len(pair.forward.sequence) - var.pos) if hit.strand == 1 else (var.pos - hit.start)
                ...
                for hit in hits_r:
                    ...
                    for var in var_filter.variants:
                        if hit.start <= var.pos <= hit.start + len(pair.reverse.sequence):
                            if var.maf >= maf:
                                dist = (var.pos - hit.start) if hit.strand == -1 else (hit.start + len(pair.reverse.sequence) - var.pos)
        ...
        total_penalty = pair.penalty + (off_target_f + off_target_r) * 10.0 + variant_penalty
    ...
    if multiplex:
        ...
        click.echo("=" * 80)
        click.echo(f"  Target Plex Limit: {num_return}-plex | Selected Compatible Loci: {len(selected_pairs)}")
        ...
        for r_idx, item in enumerate(selected_pairs, 1):
            ...
            click.echo(f"  Reverse: {pair.reverse.sequence} (Tm={pair.reverse.tm:.1f}°C, GC={pair.reverse.gc_percent:.1f}%)")
            click.echo(f"  Product Size: {pair.product_size}bp | Cross Dimer: {pair.cross_dimer_dg:.2f} kcal/mol")
            click.echo(f"  Metrics: Off-Targets={item['off_targets']} | Base Penalty={pair.penalty:.2f}")
        click.echo("=" * 80 + "\n")
    else:
        ...
        for r_idx, item in enumerate(top_n, 1):
            ...
            click.echo(f"  Reverse: {pair.reverse.sequence} (Tm={pair.reverse.tm:.1f}°C, GC={pair.reverse.gc_percent:.1f}%)")
            click.echo(f"  Product Size: {pair.product_size}bp | Cross Dimer: {pair.cross_dimer_dg:.2f} kcal/mol")
            click.echo(f"  Metrics: Off-Targets={item['off_targets']} | Variant Penalty={item['variant_penalty']:.1f} | Base Penalty={pair.penalty:.2f}")
...
def active_learn(
    ...
    click.echo(f"  Strategy: {strategy} vs. random | Batch Size: {batch_size} | Iterations: {iterations}")
    ...
    np.random.seed(42)
    bases = ["A", "T", "G", "C"]
    target_seq = "".join(random.choices(bases, weights=[0.25, 0.25, 0.25, 0.25], k=1200))

    # 2. Design candidate primer pairs
    biophys_engine = BiophysicsEngine(min_tm=54.0, max_tm=66.0, min_size=18, max_size=25)
    candidates = biophys_engine.generate_candidates(target_seq, num_return=300)
    
    if len(candidates) < 100:
        click.echo("Error: Could not generate enough candidate primers for active learning simulation.")
        return
    ...
    for strat in strategies_to_test:
        ...
        click.echo(f"  Baseline (Seeds={len(seed_set)}) | Val ROC AUC: {baseline_auc:.4f}")
        
        auc_history = [baseline_auc]

        # Active Learning iterations loop
        for itr in range(iterations):
            engine.query_and_label_next_batch(batch_size=batch_size, strategy=strat, deterministic=True)
            engine.retrain_ensemble()
            auc = eval_roc_auc(scorer_tmp, val_set)
            click.echo(f"  Iteration {itr+1}/{iterations} (Pool size={len(engine.labeled_pool)}) | Val ROC AUC: {auc:.4f}")
    ...
    for strat, hist in histories.items():
        ...
        plt.plot(x_axis, hist, label=label_text, color=colors.get(strat, "#a78bfa"), linestyle=styles.get(strat, "-"), linewidth=2.5, marker='o')

    plt.title("Active Learning Convergence Comparison", color="#e2e8f0", fontsize=14, fontweight='bold', pad=15)
