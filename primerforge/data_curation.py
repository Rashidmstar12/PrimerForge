...
class DataCurationPipeline:
    ...
    def _compute_biophysical_features(self, f_seq: str, r_seq: str) -> Dict[str, float]:
        ...
        f_gc = sum(1 for b in f_seq if b in "GC") / len(f_seq) * 100.0 if f_seq else 50.0
        r_gc = sum(1 for b in r_seq if b in "GC") / len(r_seq) * 100.0 if r_seq else 50.0
    ...
    def generate_empirical_db(self, n_samples: int = 30000) -> pd.DataFrame:
        ...
        """
        logger.info(f"Generating PrimerForge-Empirical-DB with N={n_samples} primer pairs...")
        ...
        def _calc_success(tm_diff, f_hairpin, r_hairpin, cross_dimer, f_off_targets, r_off_targets, f_var_dist, r_var_dist):
            success = 0.98
        ...
        for _ in range(n_pos):
            ...
            success = _calc_success(tm_diff, f_hairpin, r_hairpin, cross_dimer, f_off_targets, r_off_targets, f_var_dist, r_var_dist)

            # Randomly select a polymerase and salt concentration for positive cases
            p_poly = np.random.choice(["Standard_Taq", "HotStart_Taq", "HighFidelity_Phusion", "Q5"])
            p_poly_encoded = {"Standard_Taq": 0.0, "HotStart_Taq": 1.0, "HighFidelity_Phusion": 2.0, "Q5": 3.0}[p_poly]
            ...
            records.append(
                self._build_record(
                    sp, ch, f_tm, r_tm, tm_diff, f_hairpin, r_hairpin, f_homodimer, r_homodimer, cross_dimer,
                    ...
                    f_off_targets, r_off_targets, f_var_dist, r_var_dist, success,
                    p_salt_mono, p_salt_div, p_dntp, p_poly_encoded
                )
            )
        ...
        # =========================================================================
        logger.info(f"Synthesizing {n_neg} hard negative-like assays with controlled flaws...")
        neg_types = ["3_prime_clamp", "strong_dimer", "extreme_tm_diff", "structure_and_off_targets", "variant_dropout"]
        ...
        for idx in range(n_neg):
            ...
            n_poly = np.random.choice(["Standard_Taq", "HotStart_Taq", "HighFidelity_Phusion", "Q5"])
            n_poly_encoded = {"Standard_Taq": 0.0, "HotStart_Taq": 1.0, "HighFidelity_Phusion": 2.0, "Q5": 3.0}[n_poly]
            ...
            records.append(
                self._build_record(
                    ...
                    target_mfe, target_gc, target_len, primer_overlap,
                    f_off_targets, r_off_targets, f_var_dist, r_var_dist, success,
                    n_salt_mono, n_salt_div, n_dntp, n_poly_encoded
                )
            )
    ...
    def _build_record(
        self, species: str, chromosome: str, f_tm: float, r_tm: float, tm_diff: float,
        f_hairpin: float, r_hairpin: float, f_homodimer: float, r_homodimer: float, cross_dimer: float,
        f_gc: float, r_gc: float, f_len: float, r_len: float, f_clamp_gc: float, r_clamp_gc: float,
        f_poly_run: float, r_poly_run: float, f_3_dinuc_gc: float, r_3_dinuc_gc: float,
        f_3_dinuc_aa: float, f_3_dinuc_tt: float, r_3_dinuc_aa: float, r_3_dinuc_tt: float,
        f_3_stability: float, r_3_stability: float, target_mfe: float, target_gc: float,
        target_len: float, primer_overlap: float, f_off_targets: float, r_off_targets: float,
        f_var_dist: float, r_var_dist: float, success: float,
        salt_mono: float = 50.0, salt_div: float = 1.5, dntp: float = 0.2, poly_encoded: float = 0.0
    ) -> Dict[str, Any]:
        """Assembles variables into a normalized dataset row structure."""
        return {
            "species": species,
            "chromosome": chromosome,
            "f_tm": f_tm, "r_tm": r_tm, "tm_diff": tm_diff,
            "f_hairpin_dg": f_hairpin, "r_hairpin_dg": r_hairpin,
            "f_homodimer_dg": f_homodimer, "r_homodimer_dg": r_homodimer,
            "cross_dimer_dg": cross_dimer,
            "f_gc": f_gc, "r_gc": r_gc, "f_len": f_len, "r_len": r_len,
            "f_clamp_gc": f_clamp_gc, "r_clamp_gc": r_clamp_gc,
            "f_poly_run": f_poly_run, "r_poly_run": r_poly_run,
            "f_3_dinuc_gc": f_3_dinuc_gc, "r_3_dinuc_gc": r_3_dinuc_gc,
            "f_3_dinuc_aa": f_3_dinuc_aa, "f_3_dinuc_tt": f_3_dinuc_tt,
            "r_3_dinuc_aa": r_3_dinuc_aa, "r_3_dinuc_tt": r_3_dinuc_tt,
            "f_3_stability": f_3_stability, "r_3_stability": r_3_stability,
            "target_mfe": target_mfe, "target_gc": target_gc,
            "target_len": target_len, "primer_overlap": primer_overlap,
            "f_off_targets": f_off_targets, "r_off_targets": r_off_targets,
            "f_var_dist": f_var_dist, "r_var_dist": r_var_dist,
            "success": success,
    ...
    def partition_and_save(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        """
        logger.info("Partitioning dataset using chromosomal & species-stratified splits...")
        ...
        logger.info(f"Dataset split: Train N={len(train_df)} ({len(train_df)/len(df)*100:.1f}%) | Test N={len(test_df)} ({len(test_df)/len(df)*100:.1f}%)")
        ...
        df.to_csv(os.path.join(self.data_dir, "primerforge_empirical_db.csv"), index=False)
        ...
        logger.info(f"Empirical database and split matrices saved successfully to data/ directory.")
    ...
    def scrape_rtprimerdb_real(self, max_records: int = 5000) -> pd.DataFrame:
        ...
        def _normalise(raw):
            ...
            if ct_col:
                out["ct_value"] = (
                    ...
                    .fillna(25.0).clip(10.0, 40.0)
                )
            else:
                out["ct_value"] = 25.0
            eff_score = out["efficiency"].apply(
                lambda e: 1.0 if 0.90 <= e <= 1.05 else (0.6 if 0.80 <= e <= 1.15 else 0.2)
            )
            ...
            out["organism"]   = raw[org_col].astype(str).str.strip() if org_col else "unknown"
            out["gene_name"]  = raw[gene_col].astype(str).str.strip() if gene_col else "unknown"
        ...
        try:
            ...
            logger.error(f"RTPrimerDB normalisation failed: {exc}. Returning empty DataFrame.")
            return pd.DataFrame(columns=[
                "forward_seq", "reverse_seq", "success_idx", "efficiency",
                "ct_value", "polymerase", "organism", "gene_name", "source_db",
            ])
    ...
    def scrape_primerbank_real(self, max_records: int = 5000) -> pd.DataFrame:
        ...
        def _normalise(raw: pd.DataFrame) -> pd.DataFrame:
            ...
            if fwd_col is None or rev_col is None:
                raise ValueError("Could not locate Forward/Reverse columns in PrimerBank TSV.")
            ...
            out["gene_name"]  = raw[gene_col].astype(str).str.strip() if gene_col else "unknown"
            out["accession"]  = raw[acc_col].astype(str).str.strip()  if acc_col  else "unknown"
            out["organism"]   = (
                raw[species_col].astype(str).str.strip() if species_col else "Homo sapiens"
            )
        ...
        try:
            ...
            return pd.DataFrame(columns=[
                "forward_seq", "reverse_seq", "success_idx", "efficiency",
                "ct_value", "polymerase", "gene_name", "organism",
                "accession", "source_db",
            ])
    ...
    def prepare_hybrid_training_data(self) -> pd.DataFrame:
        ...
        bases = ["A", "T", "G", "C"]
        df_synth["forward_seq"] = ["".join(random.choices(bases, k=20)) for _ in range(len(df_synth))]
        df_synth["reverse_seq"] = ["".join(random.choices(bases, k=20)) for _ in range(len(df_synth))]
...
def _scrape_compat(self, target_size: int = 1000) -> pd.DataFrame:
    ...
    if len(df) > target_size:
        ...
        return pd.concat([df] * (target_size // len(df) + 1), ignore_index=True).iloc[:target_size].reset_index(drop=True)
    return df
...
class GelVisionAnalyzerStub:
    ...
    def analyze_gel_image(image_bytes: bytes) -> Dict[str, Any]:
        ...
        """
        if not image_bytes:
            return {"specificity_score": 0.0, "detected_bands": [], "gel_outcome": "Failed"}
...
class MeltCurveAnalyzerStub:
    ...
    def analyze_melt_curve(raw_telemetry: List[float]) -> Dict[str, Any]:
        ...
        for i in range(1, len(raw_telemetry) - 1):
            if raw_telemetry[i] > raw_telemetry[i - 1] and raw_telemetry[i] > raw_telemetry[i + 1]:
                peaks.append({"temp_c": 50 + i * 0.5, "derivative_fluorescence": raw_telemetry[i]})
        ...
        return {
            ...
            "specificity_index": 0.99 if single_peak else (0.30 if len(peaks) > 1 else 0.01)
        }
