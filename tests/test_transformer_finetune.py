...
def make_transformer() -> DNATransformerEncoder:
    ...
    return DNATransformerEncoder(vocab_size=8, embed_dim=16, num_heads=2, hidden_dim=32, max_len=24)
...
def test_gradient_clipping() -> None:
    ...
    assert norm_clipped <= head.clip_norm + 1e-5, (
    ...
    assert np.allclose(unclipped, small_grad, atol=1e-7), (
...
def test_fine_tune_returns_epoch_losses() -> None:
    ...
    for n_epochs in [1, 5, 10]:
        ...
        loss_history = head.fine_tune(transformer, seqs, labels, epochs=n_epochs, batch_size=8)
...
def test_serialization_round_trip() -> None:
    ...
    assert head1._n_updates == head2._n_updates, (
    ...
    assert abs(p1 - p2) < 1e-6, f"Predictions differ after round-trip: {p1:.6f} vs {p2:.6f}"
