...
def test_position_specific_weights(biophysics) -> None:
    ...
    template_3prime_mismatch = (
        "GTGGTAACCGTTACTCGCCG"  # T-G mismatch at the 3' terminal base
    )
    penalty_3prime = biophysics.calculate_mismatch_penalty(
        primer, template_3prime_mismatch
    )
    ...
    penalty_5prime = biophysics.calculate_mismatch_penalty(
        primer, template_5prime_mismatch
    )
    ...
    penalty_cc_3prime = biophysics.calculate_mismatch_penalty(
        primer_cc, "GTGGTAACCGTTACTCGCCC"
    )  # C-C mismatch at 3' end
    penalty_cc_5prime = biophysics.calculate_mismatch_penalty(
        primer_cc, "ATGGTAACCGTTACTCGCCG"
    )  # C-C mismatch at 5' end

    assert (
        penalty_cc_3prime > penalty_cc_5prime * 10.0
    )  # 3' mismatch is at least 10x more severe due to e^(-0.15 * 19)
...
def test_ml_scorer_variant_prediction() -> None:
    ...
    f_template_mismatch = (
        "GTGGTAACCGTTACTCGCCG"  # T-G mismatch at 3' end of forward primer
    )
    r_template_mismatch = (
        "GCGAGTCCTCCTCGTTACTG"  # T-G mismatch at 3' end of reverse primer
    )
