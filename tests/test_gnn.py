...
def test_gcn_layer_gradients() -> None:
    ...
    A_hat = np.array(
        [[0.5, 0.5, 0.0], [0.5, 0.33, 0.5], [0.0, 0.5, 0.5]], dtype=np.float32
    )
...
def test_biognn_gradients() -> None:
    ...
    for i in range(gnn.conv2.W.shape[0]):
        for j in range(gnn.conv2.W.shape[1]):
            ...
            dconv2_W_numeric[i, j] = np.sum(d_out * (pred_plus - pred_minus)) / (
                2 * eps
            )
...
def test_biognn_convergence() -> None:
    ...
    np.random.seed(42)
    sequences = [("ATGC", "GCAT"), ("CGTA", "TACG"), ("GGCC", "CCGG"), ("TTAA", "AATT")]
    targets = np.array(
        [[50.0, -1.0], [52.0, -1.5], [60.0, -4.0], [45.0, 0.5]], dtype=np.float32
    )
