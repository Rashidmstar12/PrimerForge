...
def test_greedy_panel_selection_and_rescue(optimizer) -> None:
    ...
    f_bad = PrimerSequence("ACCGCTCATTGCCAATGGTG", 0, 20, 60.0, 50.0, -0.2, -0.3, 0.0)  # perfect complement of f1
    r_bad = PrimerSequence("GCATGGAGTCCTGTGGCATC", 100, 20, 60.0, 50.0, -0.2, -0.3, 0.0)
    pair_bad = PrimerPair(f_bad, r_bad, 120, -10.0, 0.0) # Bad pair due to severe dimerization
    ...
    r_good = PrimerSequence("GCATGGAGTCCTGTGGCATC", 100, 20, 60.0, 50.0, -0.2, -0.3, 0.0)
    ...
    assert panel.pairs[1] == pair_good  # Successfully selected Candidate B and filtered out Candidate A!
